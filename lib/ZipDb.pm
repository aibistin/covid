#===============================================================================
#
#  DESCRIPTION: This Module manages a very trivial New York City Zip Code
#               database stored as a JSON file.
#       AUTHOR: Austin Kenny
#      VERSION: 1.0
#      CREATED: 04/12/2020
#===============================================================================
...
has zip_db_json_file => (
    ...
    coerce  => 1,
    builder => sub {
        Carp::confess $_[0]->db_dir . " doesn't exist!"
          unless ( -d $_[0]->db_dir );
        $_[0]->db_dir->child($DEFAULT_DB_NAME);
    }
);
...
has zip_hash => (
    ...
    ,
    builder => sub {
        Carp::confess $_[0]->zip_db_json_file . " doesn't exist!"
          unless ( -f $_[0]->zip_db_json_file );
...
sub get_zip_codes_matching_details {
    ...
    while ( my ( $zip, $zip_detail ) = each %{ $self->zip_hash } ) {
        push @zip_codes, { $zip => $zip_detail }
          if ( ( index( lc $zip_detail->{city}, $search_str ) > -1 )
            ...
            || ( index( lc $zip_detail->{borough},  $search_str ) > -1 )
            || ( index( lc $zip_detail->{county},   $search_str ) > -1 ) );
    }
    return \@zip_codes;
}
